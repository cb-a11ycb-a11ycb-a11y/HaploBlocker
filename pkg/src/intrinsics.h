...
typedef __m512i Ints;
typedef __m512d Doubles;
typedef __m512 Floats;
...
typedef __m256i Ints;
typedef __m256d Doubles;
typedef __m256 Floats;
...
typedef __m128i Ints;
typedef __m128d Doubles;
...
#define STOREU _mm_storeu_si128 
  ...
  #define STOREDOUBLE _mm_store_pd
  #define STOREFLOAT _mm_store_ps
#else
  #define LOAD LOADU
  #define LOADDOUBLE LOADuDOUBLE
  #define LOADFLOAT LOADuFLOAT
...
// MMX AND LOWER
// ********************************************************
#elif defined MMX || defined PlainInteger64 || defined PlainInteger32
...
#define SHL64(B,C)  (B) << (C) // unsafe for 32
#define SHR64(B,C)  (B) >> (C) // unsafe for 32
#define SHL32 SHL64 // unsafe for 64
#define SHR32 SHR64 // unsafe
#define SHL16 SHL64 // unsafe
#define SHR16 SHR32 // unsafe
#define ADD64(B,C)  (B) + (C)
#define ADD32 ADD64 // unsafe for 64
#define ANY
#define LOADU(A) *(A)
...
// MMX + 64 SPECIFIC 
// ********************************************************
#if defined MMX || defined PlainInteger64
#define SSEBITS 64U
...
#define AVAILABLE_SIMD_ASK static inline bool		\
  Available(unsigned int Blatt, unsigned int Register, int Bit) {	\
    return cpuid_info(Blatt, Register) & (1 << Bit); }

uint32_t cpuid_info(unsigned int Blatt, unsigned int Register);
...
#elif defined WINCPUID
  #define INSTALL_DEFAULT Iask
  #define AVAILABLE_SIMD AVAILABLE_SIMD_ASK
