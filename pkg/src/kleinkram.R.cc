...
SEXP TooLarge(Long n){
  int nn=stopIfNotInt(n);
  return TooLarge(&nn, 1);
}
SEXP TooLarge(Long row, Long col){
int nn[2] = {stopIfNotInt(row), stopIfNotInt(col)};
...
SEXP Int(int *V, Long n, Long max) {
  ...
  PROTECT(Ans=allocVector(INTSXP, stopIfNotInt(n)));
...
SEXP Logic(bool* V, Long n, Long max) {
  ...
  if (n>max) return TooLarge(n);
  if (n<0) return TooSmall();
  PROTECT(Ans=allocVector(LGLSXP, stopIfNotInt(n)));
  int *ans = LOGICAL(Ans);
...
SEXP Num(double* V, Long n, Long max) {
  ...
  if (n>max) return TooLarge(n);
  if (n<0) return TooSmall();
  PROTECT(Ans=allocVector(REALSXP, stopIfNotInt(n)));
...
SEXP Char(const char **V, Long n, Long max) {
  ...
  if (n>max) return TooLarge(n);
  if (n<0) return TooSmall();
  PROTECT(Ans=allocVector(STRSXP,  stopIfNotInt(n)));
...
SEXP Mat(double* V, Long row, Long col, Long max) {
  ...
  SEXP Ans;
  PROTECT(Ans=allocMatrix(REALSXP, stopIfNotInt(row), stopIfNotInt(col)));
  MEMCOPY(REAL(Ans), V, n * sizeof(*V));
  UNPROTECT(1);
  return Ans;
}
...
SEXP MatString(char **V, Long row, Long col, Long max) {
  ...
  if (n>max) return TooLarge(row, col);
  SEXP Ans;
  PROTECT(Ans=allocMatrix(STRSXP, stopIfNotInt(row), stopIfNotInt(col)));  
...
SEXP MatInt(int* V, Long row, Long col, Long max) {
  ...
  if (n>max) return TooLarge(row, col);
  SEXP Ans;
  PROTECT(Ans=allocMatrix(INTSXP, stopIfNotInt(row), stopIfNotInt(col)));
  MEMCOPY(INTEGER(Ans), V, n * sizeof(*V));
  UNPROTECT(1);
  return Ans;
}
...
SEXP Array3D(double** V, Long depth, Long row, Long col, Long max) {
  ...
  SEXP Ans;
  PROTECT(Ans=alloc3DArray(REALSXP, stopIfNotInt(depth), stopIfNotInt(row),
			   stopIfNotInt(col)));
  double *ans = REAL(Ans);
...
SEXP String(char V[][MAXCHAR], Long n, Long max) {
  ...
  if (n>max) return TooLarge(n);
  if (n<0) return TooSmall();
  PROTECT(str = allocVector(STRSXP, stopIfNotInt(n))); 
...
SEXP String(int *V, const char * List[], Long n, Long endvalue) {
  ...
  PROTECT(str = allocVector(STRSXP, stopIfNotInt(k))); 
...
bool Logical(SEXP p, char *name, Long idx) {
  if (p != R_NilValue) { assert(idx < length(p)); }
   switch (TYPEOF(p)) {
    case REALSXP:
...
void GetName(SEXP el, char *name, const char * List[], int n,
  ...
  if (len_el > maxlen_ans) 
    RFERROR2("option '%.50s' is too lengthy. Maximum length is %u.",
	     name, maxlen_ans);
