...
void strcopyN(char *dest, const char *src, Long n) {
  if (n > 1) {
    n--; 
    strncpy(dest, src, n);
  }
...
#pragma omp parallel for num_threads(GreaterZero(cores)) schedule(dynamic, 20) if (MULTIMINSIZE(ncol)) 
  ...
  default :
    double alpha = 1.0,
      beta = 0.0;
    int ncol0 = stopIfNotInt(ncol),
      nrow0 = stopIfNotInt(nrow);
...
void XCXt(double *X, double *C, double *V, Long nrow, Long dim /* dim of C */, int VARIABLE_IS_NOT_USED cores) {
  Long size = nrow * dim;
  double *endpX = X + nrow;
  double *tmp = (double*) MALLOC(sizeof(double) * size); // tmp = XC
...
int Match(char *name, const char * List[], int n, char sep, int *Len) {
  ...
  while ( Nr < n) {
	...
	if (j<n) {
	  ...
	  if (ln==stopIfNotInt(tmp)) return j;  // exactmatching -- take first 
	  else multiplematching=true;
	}
